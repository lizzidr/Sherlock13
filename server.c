...
int main(int argc, char *argv[])
{
        ...
        if (fsmServer==0)
        {
        	switch (buffer[0])
        	{
                ...
                if (nbClients==4)
				{
					...
					// On envoie ses cartes au joueur i (0, 1, 2, 3), ainsi que la ligne qui lui correspond dans tableCartes
					for (i = 0; i < 4; i++) {
                        // cartes du joueur i 
                        sprintf(reply, "D %d %d %d", deck[i*3], deck[i*3+1], deck[i*3+2]);
                        sendMessageToClient(tcpClients[i].ipAddress, tcpClients[i].port, reply);
                        // sa ligne de tableCartes 
                        for (j = 0; j < 8; j++) {
	...
	else if (fsmServer==1)
	{
		switch (buffer[0])
		{
				...
				// RAJOUTER DU CODE ICI
						int obj;
						sscanf(buffer, "O %d %d", &pid, &obj);
						/* on envoie au demandeur la valeur tableCartes de chaque joueur pour cet objet */
						for (i = 0; i < 4; i++) {
							sprintf(reply, "V %d %d %d", i, obj, tableCartes[i][obj]);
				...
				break;
					case 'S':
						// RAJOUTER DU CODE ICI
						int cible;
						sscanf(buffer, "S %d %d %d", &pid, &cible, &obj);
						/* on envoie au demandeur juste la valeur pour le joueur ciblé et l’objet */
						sprintf(reply, "V %d %d %d",
								cible, obj, tableCartes[cible][obj]);
